...
from mindrake.agents.baseline_sub_agents.neural_nets import *
import os
from mindrake.agents.baseline_sub_agents.sub_agents import *
import os.path as path
from CybORG.Agents import B_lineAgent, SleepAgent, RedMeanderAgent
from mindrake.agents.baseline_sub_agents.configs import *
# from CybORGActionAgent import CybORGActionAgent
import ray.rllib.agents.ppo as ppo
from ray.rllib.agents.ppo import PPOTrainer
from ray.rllib.models import ModelCatalog
from mindrake.agents.baseline_sub_agents.bline_CybORGAgent import CybORGAgent as bline_CybORGAgent

# Added custom?
from ray.rllib.algorithms.ppo import ppo_torch_policy
...
class LoadBanditBlueAgent:
    ...
    def get_action(self, obs, action_space):
        ...
        # print("loadBanditController.py - get_action()")
        # print("obs is ", print(obs))
        # print("type(obs) is ", print(type(obs)))
        # print("len(obs) is ", print(len(obs)))
        ...
        if self.step < 5:
            ...
            # Sometimes this line will cause a key error; thus try and default to Bline
            try:
                self.adversary = np.argmax(self.controller[bandit_obs_hashable])
            except KeyError:
                print("KeyError detected in loadBanditController.py - get_action()")
                self.adversary = -2
        ...
        # Force agent behavior here
        # Setting to sleep to determine how the competitive red agent works
        self.adversary = 2

        if self.adversary == 0 or self.adversary == -3:
        ...
        elif self.adversary == 1 or self.adversary == -2:
    ...
    def end_episode(self):
        # print("loadBanditController.py - end_episode())")
        self.set = False
        self.state = [np.zeros(256, np.float32),
               np.zeros(256, np.float32)]
        self.bandit_observation = np.array([], dtype=int)
